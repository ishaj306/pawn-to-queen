...
## When Clerk lands (next session)

Right now policies rely on Supabase Auth's `auth.uid()`. After the Clerk
migration we'll configure a **Clerk JWT template** named `supabase` that
signs tokens with Supabase's JWT secret and sets `sub` to the user's
Clerk ID (mapped to a stable UUID via a custom claim). With that template
in place, `auth.uid()` keeps returning the same UUID and **none of these
policies have to change**. See `project_clerk_migration.md` in memory.

If you'd rather pass the user ID explicitly from server actions, drop
RLS entirely and gate inserts/selects in the action layer — but that's
a step backward in security.
